DROP TABLE IF EXISTS transformed.categorytagtranslation;
DROP TABLE IF EXISTS transformed.categorytagforce;
...
CREATE TABLE Transformed.Article (
    ...
    categories TEXT[],
    source TEXT, -- Shop e.g
    transformationTime TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP  
);
...
CREATE TABLE transformed.categorytagsdictionary (
    name TEXT PRIMARY KEY,
    preference INTEGER
);


CREATE TABLE transformed.categorytagtranslation (
    alias TEXT PRIMARY KEY,
    category TEXT NOT NULL REFERENCES transformed.categorytagsdictionary (name),
    userid UUID,
    createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE transformed.categorytagforce (
    productname TEXT REFERENCES transformed.productnamedictionary (name),
    brand TEXT REFERENCES transformed.branddictionary (name),
    category TEXT NOT NULL REFERENCES transformed.categorytagsdictionary (name),
    userid UUID,
    createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

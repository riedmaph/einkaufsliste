...
describe('Markets', () => {
  ...
  describe('/GET markets for Franz Wolter Strasse in 100m', () => {
    it('it should return no markets', (done) => {
      chai.request(app)
          .get('/api/markets/search?latitude=48.174805&longitude=11.633389&max-distance=100')
  ...
  describe('/GET markets for Franz Wolter Strasse in 240m', () => {
    it('it should return one market', (done) => {
      chai.request(app)
          .get('/api/markets/search?latitude=48.174805&longitude=11.633389&max-distance=240')
          .set('x-access-token', token)
          .end((err, res) => {
              res.should.have.status(200);
              res.body.should.be.a('object');
  ...
  describe('/GET markets for Franz Wolter Strasse in 250m', () => {
    it('it should return two markets', (done) => {
      chai.request(app)
          .get('/api/markets/search?latitude=48.174805&longitude=11.633389&max-distance=250')
          .set('x-access-token', token)
          .end((err, res) => {
              res.should.have.status(200);
              res.body.should.be.a('object');

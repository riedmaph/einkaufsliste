...
SELECT a.id, a.title,
    COALESCE(NULLIF(cleanString(a.name),''),mlp.productname), COALESCE(mlp.likelihood,1), CASE WHEN a.name IS NOT NULL THEN 'original' WHEN mlp.productname IS NOT NULL THEN 'mining' ELSE NULL END,
